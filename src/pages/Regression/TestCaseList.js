        ...
        >
            <FormItem labelCol={{ span: 5 }} wrapperCol={{ span: 15 }} label="测试集">
                {form.getFieldDecorator('datasetId', {
                    rules: [{ required: true, message: '请选择试集！' }],
            ...
            <FormItem labelCol={{ span: 5 }} wrapperCol={{ span: 15 }} label="请求数据">
                {form.getFieldDecorator('data', {
                    rules: [{ required: true }],
                })(<TextArea placeholder="请输入请求数据并以逗号分隔" />)}
            </FormItem>
            <FormItem labelCol={{ span: 5 }} wrapperCol={{ span: 15 }} label="期望结果">
                {form.getFieldDecorator('expectData', {
                    rules: [{ required: true }],
                })(<TextArea placeholder="请输入期望结果并以逗号分隔" />)}
            </FormItem>
...
class UpdateForm extends PureComponent {
    ...
    constructor(props) {
        ...
        this.state = {
            formVals: {
                ...
                expectData: props.values.expectData,
                weight: props.values.weight,
                testDatasetsInfo: { id: props.values.testDatasetsInfo.id},
            },
        };
    }
    ...
    renderContent = (formVals) => {
        const { form, setList } = this.props;
        
        return [
            <FormItem key="datasetId" labelCol={{ span: 5 }} wrapperCol={{ span: 15 }} label="测试集">
                {form.getFieldDecorator('datasetId', {
                    rules: [{ required: true, message: '请输入测试集！' }],
                    initialValue: formVals.testDatasetsInfo.id,
                })(<Select placeholder="请选择" style={{ width: '100%' }}>
                    {
                        setList.length && setList.map(item => (
                            <Select.Option key={item.id} value={item.id}>{item.name}</Select.Option>)
            ...
            <FormItem key="data" labelCol={{ span: 5 }} wrapperCol={{ span: 15 }} label="请求数据">
                {form.getFieldDecorator('data', {
                    rules: [{ required: true }],
                    initialValue: formVals.data,
                })(<TextArea placeholder="请输入请求数据并以逗号分隔" />)}
            </FormItem>,
            <FormItem key="expectData" labelCol={{ span: 5 }} wrapperCol={{ span: 15 }} label="期望结果">
                {form.getFieldDecorator('expectData', {
                    rules: [{ required: true }],
                    initialValue: formVals.expectData,
                })(<TextArea placeholder="请输入期望结果并以逗号分隔" />)}
            </FormItem>,
            <FormItem key="weight" labelCol={{ span: 5 }} wrapperCol={{ span: 15 }} label="权重">
                {form.getFieldDecorator('weight', {
                    rules: [{ required: false }],
...
@Form.create()
class TestCaseList extends PureComponent {
    paginationProps = {
        currentPage: 1,
        pageSize: 10,
    };
    ...
    columns = [
        ...
        },
        {
            ...
            dataIndex: 'data',
            render: text =>
                <Popover content={text} title="请求数据" trigger="hover">
